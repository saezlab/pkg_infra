...
dependencies needed for running pkg_infra (`[tool.poetry.dependencies]` with the
...
developing pkg_infra using test-driven development. Please make sure that you
...
If you are brand new to pkg_infra or open-source development, we recommend
...
pkg_infra is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of pkg_infra

You will need your own fork of pkg_infrain order to eventually open a Pull
Request. Go to the pkg_infra project page and hit the Fork button. Please
...
This creates the directory `pkg_infra` and connects your repository to the
upstream (main project) *pkg_infra* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of pkg_infra
repository. First ensure it's up-to-date with the main pkg_infra repository.
...
to create a branch called `my-new-feature-for-pkg_infra`

```bash
git checkout -b my-new-feature-for-pkg_infra
```

This changes your working branch from `main` to the
`my-new-feature-for-pkg_infra` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *pkg_infra*. You
...
When making changes, follow these pkg_infra-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the pkg_infra project.
...
1. Write a descriptive title that includes prefixes. pkg_infra uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the pkg_infra `main` branch, run:

```shell

    git checkout my-new-feature-for-pkg_infra
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-pkg_infra
```
